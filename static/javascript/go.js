...
function Board(size) {
  ...
  for (var x = 0; x < this.size; x++) {
      this.stones[x] = new Array(this.size);
  ...
  this.sgf = new sgf_format.create_sgf_formatter(this.size);  // Maybe use this as history later.
...
this.get_sgf = function() { return this.sgf.get_sgf_string(); };
  ...
  this.place_score_stones = function() {
      ...
      for (var x = 0; x < this.size; x++) {
          for (var y = 0; y < this.size; y++) {
              if (this.stones[x][y] === null) {
                  var score_stone = new Stone(x, y, -1); //Add stone of neutral colour
  ...
  this.area_score = function() {
      ...
      while(score_stones.length >= 1){
          var active_stone = score_stones.pop();
      ...
      var placed_stones = this.count_player_stones();
  ...
  this.determine_winner = function() {
      ...
      if(score.black > score.white)
          ...
          this.winner = -1; // Draw
      var winner_string = (this.winner == 1) ? 'Black' : 'White'; //FIXME delete later
  ...
  this.remove_dead_marks = function(mark_array) {
      while(mark_array.length() > 0) {
          var mark = mark_array.pop();
          var stone_color = this.stones[mark.x][mark.y].color;
          if(stone_color == 1)
              this.cap_white += 1;
          else
              this.cap_black += 1;
